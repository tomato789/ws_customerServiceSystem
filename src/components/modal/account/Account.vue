<template>
    <div class="container">
        <el-dialog v-model="visible" title="群发好友" width="1000" :top="'5vh'">
            <div>
                <div class="statistics">
                    <div class="title">引流统计总览</div>
                    <el-row>
                        <el-col :span="6">
                            <div>0/0</div>
                            <div>引流总数/去重后</div>
                ...
                <x-table :listView="listView" @lodaData="lodaData" @selectionChange="selectionChange">
                    <template #opreaLeft>
                        <el-button type="primary">公共组取号</el-button>
                        <el-button type="primary">建群</el-button>
                        <el-button type="primary">账号注册</el-button>
                        <el-button type="primary">离线登录</el-button>
                        <el-button type="primary">离线登录日志</el-button>
                        <el-button type="primary">好友转移</el-button>
                    </template>
                    <template #opreaRight>
                        <el-dropdown>
                            <el-button type="primary">
                                批量操作账号<el-icon class="el-icon--right"><arrow-down /></el-icon>
                            </el-button>
                            <template #dropdown>
                                <el-dropdown-menu>
    ...
    if (Object.keys(listView.params).length > 0) {
        ...
        listView.data = data.filter((item: any) => {
            for (const key in listView.params) {
                if (item[key] && item[key].indexOf((listView.params as any)[key]) === -1) {
                    return false;
                }
            }
            return true;
